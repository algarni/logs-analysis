...
query = """
	...
	FROM 
		(SELECT date(time) AS log_date, COUNT(*) AS log_count FROM log GROUP BY log_date) AS a, 
		(SELECT DATE(time) AS log_date, COUNT(*) AS log_count FROM log WHERE status LIKE '404%' GROUP BY log_date) AS b 
